...
# Select from the database all of the TIMES the tweets you collected were posted and fetch all the 
# tuples that contain them in to the variable tweet_posted_times.
times = "SELECT time_posted FROM Tweets"
cur.execute(times)
...
# (i.e. have "RT" at the beginning of the tweet text). Save the FIRST ONE from that group of text 
# values in the variable first_rt. Note that first_rt should contain a single string value, not a tuple.
y = "SELECT tweet_text FROM Tweets WHERE instr(tweet_text, 'RT')"
cur.execute(y)
y1 = cur.fetchone()
first_rt = y1[0]
# print(first_rt)
...
# Define a function get_twitter_users that accepts a string as in put and returns a SET of 
# the _twitter screennames_ of each twitter user who was mentioned in that string. 

# Note that the syntax for mentions in a tweet is that the username is preceded by an "@" character, e.g. 
# "@umsi" or "@aadl", and cannot contain any punctuation besides underscores -- that's how to determine what 
# user names are mentioned. (e.g. @hello? is just the username "hello", but @programmer_at_umsi is 
# "programmer_at_umsi"). 
...
# You may assume for this problem that there will be no usernames directly in order, e.g. 
# "@hello@goodbye", only "@hello and @goodbye" for example. We will iterate on this later!

# Also note that the SET type is what this function should return, NOT a list or tuple. We looked at 
